...
private:
  ...
  void handleBtn(int btn, bool &btnState, void (*callback)(UniversalRemote *)) {
    ...
    if (currentBtnState == LOW && btnState == HIGH) {
      callback(this);  // Pass 'this' to callback
      btnState = LOW;
    } else if (currentBtnState == HIGH && btnState == LOW) {
      btnState = HIGH;
    }
  }
...
public:
  ...
  void captureSignal() {
    currentRawDataLen = IrReceiver.decodedIRData.rawlen;

    if (currentRawDataLen > 200) {
      currentRawDataLen = 200;
    }

    // Just store the protocol and data without accessing raw buffer
    // This works if you only need to resend using the library's built-in methods
    for (uint8_t i = 0; i < currentRawDataLen && i < 200; i++) {
      currentRawData[i] = 0;  // Initialize
    }
  ...
  IRSignal prontoToRaw(const char *name, const uint16_t prontoHex[], size_t length) {
    IRSignal signal;
    strncpy(signal.name, name, sizeof(signal.name));

    if (length < 4)
      return signal;  // invalid

    uint16_t freqCode = prontoHex[1];
    double frequency = 1000000.0 / (freqCode * 0.241246);
    double unit = 1000000.0 / frequency / 0.241246;

    size_t dataStart = 4;
    size_t rawLen = length - dataStart;
    if (rawLen > 200)
      rawLen = 200;

    for (size_t i = 0; i < rawLen; i++) {
      signal.rawData[i] = (uint16_t)(prontoHex[dataStart + i] * unit);
    }

    signal.rawDataLen = rawLen;
    return signal;
  }

  void sendFixSignal(uint16_t *signalData) {
    IrSender.sendRaw(signalData, sizeof(signalData) / sizeof(signalData[0]), 38);
  }
  ...
  void menuSetup() {
    ...
    menuShown = true;
    refreshScreen();
    printText(2, ST7735_WHITE, 15, 10, "FREEZE test");
  }
  ...
  void confirmSelection() {
    ...
    if (cursorRow == 2 && cursorCol == 7) {
      ...
      if (cursorCol == 1)
        charIndex = 25;  // "N"
      else if (cursorCol == 2 || cursorCol == 3)
        charIndex = 23;  // "_"
      else if (cursorCol == 4)
        charIndex = 27;  // "<"
      else if (cursorCol == 5)
  ...
  static void epsonFreeze(UniversalRemote *remote){
    remote->sendFixSignal(EPSON_CODES[0]);
  }

  static void acerFreeze(UniversalRemote *remote){
    remote->sendFixSignal(ACER_CODES[0]);
  }

  static void benqFreeze(UniversalRemote *remote){
    remote->sendFixSignal(BENQ_CODES[0]);
  }

  static void necFreeze(UniversalRemote *remote){
    remote->sendFixSignal(NEC_CODES[0]);
  }

  static void panasonicFreeze(UniversalRemote *remote){
    remote->sendFixSignal(PANASONIC_CODES[0]);
  }
